...
//    public static final String OVERVIEW = "OVERVIEW", WORKSHOP = "WORKSHOP";
    ...
    public String currentWorkShopSubMode = SUB_WORKSHOP_MODE_NONE; //determine what sub mode workshop is in
...
//        displayPersonInfoList(overviewContainer, mainInfoTooltipKey, workshopMidAndRightW, pH, MAIN_CONTAINER_PADDING_X/2, MAIN_CONTAINER_PADDING_Y/2);
    ...
    protected void displayWorkshopMidTop(ba_component creatorComponent, String creatorComponentTooltip, float cW, float cH, float cX, float cY) {
        ...
        if(isDisplayingOtherFleets) {
            LabelAPI header = tooltipContainer.addPara("Mode: %s", 0f, Misc.getBrightPlayerColor(),"Preview");
...
//            }
    ...
    protected void displayWorkshopMidBottom(ba_component creatorComponent, String creatorComponentTooltip, float cW, float cH, float cX, float cY) {
        ...
        BRMUnderline.getPosition().setSize(limitBRMLabel.computeTextWidth("BRM:" + limitBRM + " / " + currentBRM) + pad, limitBRMLabel.computeTextHeight("BRM: 0000 / 0000") + pad);
        ...
        personStatsTooltip.addTooltipTo(new TooltipMakerAPI.TooltipCreator() {
            ...
            @Override
            public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
                tooltip.addSectionHeading("Bionic Right Management", Alignment.MID, 0);
                tooltip.addPara("Limit: %s", pad, Misc.getBrightPlayerColor(), ""+limitBRM).setAlignment(Alignment.MID);
                tooltip.addPara("Usage: %s", pad, Misc.getHighlightColor(), ""+currentBRM).setAlignment(Alignment.MID);
                tooltip.addPara("Available: %s", pad, (limitBRM - currentBRM)>0?Misc.getPositiveHighlightColor():Misc.getNegativeHighlightColor(), ""+(limitBRM - currentBRM)).setAlignment(Alignment.MID);
                ...
                tempBioformData.sort((o1, o2) -> o1.bionicInstalled.brmCost < o2.bionicInstalled.brmCost ? 1 : 0);
                ...
                if(tempBioformData.isEmpty()) {
                    ...
                    for(ba_officermanager.ba_bionicAugmentedData data: tempBioformData) {
                        ...
                        tooltip.addToGrid(3, i, "", "" + ( (int) data.bionicInstalled.brmCost), Misc.getNegativeHighlightColor());
                        i++;
                    }
                }
                tooltip.addGrid(pad);
                if(expanded) {
                    if (!currentPerson.isAICore()) {
                        tooltip.addPara(ba_stringhelper.getString("BRM", "BRM_explain"), Misc.getGrayColor(), 10f);
                        tooltip.addPara(ba_stringhelper.getString("BRM", "BRM_galatia"), Misc.getGrayColor(), 10f);
                    } else {
                        tooltip.addPara(ba_stringhelper.getString("BRM", "BRM_explain_ai"), Misc.getGrayColor(), 10f);
                        tooltip.addPara("...", Misc.getGrayColor(), 10f);
                        tooltip.addPara("...", Misc.getGrayColor(), 10f);
                        tooltip.addPara("...", Misc.getGrayColor(), 10f);
                        tooltip.addPara("Override Completed...", Misc.getGrayColor(), 10f);
                    }
                }
            }
        }, BRMUnderline, TooltipMakerAPI.TooltipLocation.ABOVE);
        //>Consciousness
    ...
    protected void displayWorkshopRight(ba_component creatorComponent, String creatorComponentTooltip, float cW, float cH, float cX, float cY) {
        ...
        if(this.currentWorkShopSubMode.equals(SUB_WORKSHOP_MODE_NONE)) {
            ...
            ButtonAPI invButton = btnTooltipContainer.addButton("Inventory", null, Misc.getTextColor(), this.currentWorkshopEffectOrInvTab.equals(WORKSHOP_INV)?Misc.getDarkPlayerColor():Misc.getDarkPlayerColor().darker().darker(), Alignment.MID, CutStyle.NONE, invEffectBtnW, invEffectBtnH, 0);
...
//            invButton.setButtonDisabledPressedSound("ui_button_pressed");
            ...
            ButtonAPI effectButton = btnTooltipContainer.addButton("Effect", null, Misc.getTextColor(), this.currentWorkshopEffectOrInvTab.equals(WORKSHOP_EFFECT)?Misc.getDarkPlayerColor():Misc.getDarkPlayerColor().darker().darker(), Alignment.MID, CutStyle.NONE, invEffectBtnW, invEffectBtnH, 0);
...
//            effectButton.setButtonDisabledPressedSound("ui_button_pressed");
            ...
            if(this.currentWorkShopMode.equals(EDIT_WORKSHOP)) {
                addButtonToList(confirmButton, "bionic:confirmRemoveBionicList");
            ...
            confirmTooltipContainer.addTooltipTo(new TooltipMakerAPI.TooltipCreator() {
                ...
                @Override
                public float getTooltipWidth(Object tooltipParam) {
                    return (currentWorkShopMode.equals(INSTALL_WORKSHOP)?650f:400f);
                }
                ...
                public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
                    if(currentWorkShopMode.equals(INSTALL_WORKSHOP)) {
                        ...
                        tooltip.addPara("Button is still disabled ? Hover on selected bionic / limb for more information.", pad*2);
                        tooltip.setParaFontDefault();
                        tooltip.addPara("Make sure that: ", pad*2);
                        LabelAPI bionicInstallableLabel = tooltip.addPara("[ %s ] %s on selected limb.", pad, Misc.getHighlightColor(), isBionicInstallableOnLimb? "O": "X","Selected bionic can be installed");
                        ...
                        LabelAPI bionicInstalledLabel = tooltip.addPara("[ %s ] %s bionic installable per limb limit. The current limit is %s", pad, Misc.getHighlightColor(), !isBionicAlreadyInstalledOnLimb? "O": "X", "Selected limb is not exceeding", "" +ba_variablemanager.BIONIC_INSTALL_PER_LIMB);
                        ...
                        LabelAPI bionicPersonTypeLabel = tooltip.addPara("[ %s ] %s for the person profession (Officer/Admin)", pad, Misc.getHighlightColor(), isBionicInstallableBaseOnPersonType? "O": "X", "Selected bionic have applying effect");
                        ...
                        tooltip.addPara("Note: Player can install both type", g, pad/2);
                        LabelAPI brmLabel = tooltip.addPara("[ %s ] %s the person BRM limit.", pad, Misc.getHighlightColor(), !isBrmExceed? "O": "X","Selected bionics BRM do not go past");
                        ...
                        LabelAPI consciousnessLabel = tooltip.addPara("[ %s ] %s one's consciousness to lower than %s.", pad, Misc.getHighlightColor(), !isConsciousnessReduceToZero? "O": "X","Selected bionics consciousness cost does not reduce", "0");
                        ...
                        LabelAPI conflictedLabel = tooltip.addPara("[ %s ] %s with other bionics installed on the person.", pad, Misc.getHighlightColor(), !isBionicConflicted? "O": "X", "Selected bionic is not conflicting");
        ...
        if(this.currentWorkShopSubMode.equals(SUB_WORKSHOP_MODE_BIOFORM)) {
            ...
            displayLimbListWorkshop(container, rightInnerTooltipKey, effectListW-pad/2, effectListH/2, pad/2, 0);
            displayLimbModificationSummaryWorkshop(container, rightInnerTooltipKey, effectListW-pad/2, effectListH/2, pad/2, effectListH/2);
        }
    }
...
//        }
        ...
        if(this.currentWorkShopSubMode.equals(SUB_WORKSHOP_MODE_BIOFORM)) {
            ...
            effectListTooltipContainer.getPosition().inTL(personInfoW, 0);
            displayLimbListWorkshop(workshopContainer, mainEffectsTooltipKey, effectListW, effectListH/2, 0, 0);
            displayLimbModificationSummaryWorkshop(workshopContainer, mainEffectsTooltipKey, effectListW, effectListH/2, 0, effectListH/2);
        }
    }
...
//        subEffectListTooltipContainer.addSpacer(spacerY);
    ...
    public void displayLimbListWorkshop(ba_component creatorComponent, String creatorComponentTooltip, float W, float H, float X, float Y) {
        float pad = 10f;
        float opad = 10f;
        Color h = Misc.getHighlightColor();
        Color bad = Misc.getNegativeHighlightColor();
        ...
        String effectListTooltipKey = "LIMB_LIST_TOOLTIP";
        String effectListPanelKey = "LIMB_EFFECT_LIST_PANEL";
        ...
        //border effect list
        int borderW = (int) (W - pad);
        int borderH = (int) (H * 1f - pad - pad);
        int borderX = (int) pad/2;
        int borderY = (int) (pad);
        ...
        TooltipMakerAPI subLimbListTooltipContainer = subLimbListContainer.createTooltip(subEffectListTooltipKey, subEffectW, subEffectH, true, 0,0);
        listContainer.attachSubPanel(effectListTooltipKey, subEffectListPanelKey,subLimbListContainer,0,0);
        ...
        for(ba_limbmanager.ba_limb limb: limbList) {
            if(!ba_limbmanager.isLimbCentralLimb(limb) && ba_limbmanager.isLimbBaseLimb(limb)) {
                ...
                btn.setEnabled(getCurrentLimbPartCount() < getCurrentLimbLimit());
...
//            text.getPosition().inTL(btnW + pad, rowHeight*i);
        ...
        //do the adding late so the scroll work
        subLimbListContainer.mainPanel.addUIElement(subLimbListTooltipContainer).setLocation(0,0).inTL(subEffectX, subEffectY);
    }
    public void displayLimbModificationSummaryWorkshop(ba_component creatorComponent, String creatorComponentTooltip, float W, float H, float X, float Y) {
        float pad = 10f;
        float opad = 10f;
        Color h = Misc.getHighlightColor();
        Color bad = Misc.getNegativeHighlightColor();
        ...
        listTooltipContainer.addTooltipTo(new TooltipMakerAPI.TooltipCreator() {
            ...
            @Override
            public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
                tooltip.addSectionHeading("On confirm", Alignment.MID, 0);
                if(bioformChangeList.isEmpty()) {
                    LabelAPI empty = tooltip.addPara("Empty", Misc.getGrayColor(),pad);
...
//                    String str = String.join("",line.getValue().split("\\|  >"));
                    ...
                    String getBackBionicString2 = "";
                    Color getBackColor = g;
                    if(bioformData.bionicInstalled != null) {
                        if(!bioformData.bionicInstalled.isAllowedRemoveAfterInstall) {
                            getBackBionicString = "On remove, reduce";
                            getBackBionicString2 = "to atom";
                            getBackColor = Misc.getNegativeHighlightColor();
                        } else {
                            getBackBionicString = "On remove, return";
                            getBackBionicString2 = "as item";
                        }
...
//                    tooltip.setParaFontVictor14();
                    ...
                    if(bioformData.bionicInstalled != null) {
                        tooltip.setParaFontDefault();
                        String overclockText = (bioformData.bionicInstalled != null && ba_overclockmanager.isBionicOverclockable(bioformData.bionicInstalled))? (" "  + (bioformData.appliedOverclock!= null ? "[ "+ bioformData.appliedOverclock.name+" ]" : "[ --- ]")): "";
                        LabelAPI label = tooltip.addPara("       " + getBackBionicString + " " + bioformData.bionicInstalled.getName() + overclockText + " " + getBackBionicString2, pad/2, Misc.getTextColor(), "");
                        label.setHighlight(getBackBionicString, bioformData.bionicInstalled.getName(), "[", bioformData.appliedOverclock != null ? bioformData.appliedOverclock.name: "---", "]", getBackBionicString2);
                        label.setHighlightColors(getBackColor, bioformData.bionicInstalled.displayColor, ba_variablemanager.BA_OVERCLOCK_COLOR, bioformData.appliedOverclock != null ? Misc.getHighlightColor(): g, ba_variablemanager.BA_OVERCLOCK_COLOR, getBackColor);
                    }
                    if(bioformData.bionicInstalled != null) {
        ...
        String modificationListTooltipKey = "LIMB_MOD_SUMMARY_INNER_TOOLTIP";
        String modificationListPanelKey = "LIMB_MOD_SUMMARY_INNER_PANEL";
        int modificationH = (int) (H - header.computeTextHeight("Console") - buttonH - pad * 5);
        int modificationW = (int) (W - pad - pad);
        int modificationX = (int) (pad);
        ba_component modificationListContainer = new ba_component(componentMap, listContainer.mainPanel, modificationW, modificationH, modificationX, 0, false, modificationListPanelKey);
...
//                                    this.currentBioformData.clear();
                ...
                if(tokens[0].equals("bionic")) {
                    ...
                    if(tokens[1].equals("confirmRemoveBionicList")) {
                        for (ba_officermanager.ba_bionicAugmentedData data : this.currentRemovingBionics) {
