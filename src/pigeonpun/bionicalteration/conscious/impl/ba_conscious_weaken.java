...
public class ba_conscious_weaken implements ba_conscious {
    ...
    public float getThreshold() {
        return ba_variablemanager.BA_CONSCIOUSNESS_THRESHOLD.get(ba_variablemanager.BA_CONSCIOUSNESS_WEAKENED_THRESHOLD);
    }
    ...
    public void displayTooltipDescription(TooltipMakerAPI tooltip, PersonAPI person, boolean isActive, boolean isSimpleMode) {
        ...
        if(!isSimpleMode) {
            ...
            tooltip.addPara("%s: %s", pad, Misc.getTextColor(), "Weaken", ba_stringhelper.getString("conscious", "ba_weaken_person")).setHighlightColors(getColor(), Misc.getTextColor());
            tooltip.addSectionHeading("Effects", Alignment.MID, pad);
        } else {
            if(!isActive) textAlpha = 0.6f;
            tooltip.setParaFontOrbitron();
            tooltip.addPara("Weaken:", getColor(), pad).setOpacity(textAlpha);
        }
        tooltip.setParaFontOrbitron();
